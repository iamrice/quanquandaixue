...
Page({
  ...
  search: function(e) {
    ...
    if (this.data.selectedTag == 0 && key != '') {
      db.collection('CourseInfo').where(_.or([{
          courseData: {
      ...
      db.collection('sickHelpList').where(_.or([{
          sickHelpData: {
            courseName: db.RegExp({
              regexp: '.*' + key,
              options: 'i',
